    ...
    <body class="min-h-screen bg-page text-ui-primary antialiased">
        <div class="relative">
            ...
            <main class="guest-page-main mx-auto flex w-full max-w-7xl flex-col gap-10 px-6 pb-20 lg:px-8 lg:pb-28">
                <p class="text-sm">
                    <a href="{{ route('authors.index') }}" class="text-ui-faint underline decoration-border-strong underline-offset-4 hover:text-ui-primary">
                        {{ __('All authors') }}
                    </a>
                </p>

                <header class="space-y-4 border-b border-ui pb-6">
                    <h1 class="font-serif text-4xl font-semibold tracking-tight text-ui-primary sm:text-5xl">
                        {{ $author->name }}
                    </h1>
                    @if ($alts !== null && $alts !== [])
                        <p class="text-sm text-ui-muted">
                            <span class="font-medium text-ui-faint">{{ __('Also known as') }}:</span>
                            {{ implode(', ', $alts) }}
                        </p>
                    @endif
                    @if (filled($author->bio))
                        <p class="max-w-3xl text-base leading-7 text-ui-muted">
