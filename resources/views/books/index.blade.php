    ...
    <body class="min-h-screen bg-page text-ui-primary antialiased">
        <div class="relative">
            ...
            <main class="guest-page-main mx-auto flex w-full max-w-7xl flex-col gap-8 px-6 pb-20 lg:px-8 lg:pb-28">
                <form method="get" action="{{ route('books.index') }}" class="border-b border-ui pb-4">
                    @php
                        $filterBubble = 'inline-flex min-w-0 items-stretch overflow-hidden rounded-md border border-ui bg-surface transition focus-within:border-border-strong hover:border-border-strong';
                        $filterBubbleLabel = 'flex shrink-0 items-center border-r border-ui bg-raised px-2.5 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-ui-faint';
                        $filterSelect = 'min-w-0 flex-1 cursor-pointer appearance-none border-0 bg-transparent py-2 pl-2 pr-8 text-[11px] font-semibold uppercase tracking-wider text-ui-primary outline-none sm:min-w-[6.5rem]';
                        $filterSelectNormalCase = 'min-w-0 flex-1 cursor-pointer appearance-none border-0 bg-transparent py-2 pl-2 pr-8 text-[11px] font-semibold normal-case tracking-normal text-ui-primary outline-none sm:min-w-[6.5rem]';
                        $filterSelectDisabled = 'min-w-0 flex-1 cursor-not-allowed appearance-none border-0 bg-transparent py-2 pl-2 pr-8 text-[11px] font-semibold uppercase tracking-wider text-ui-faint outline-none sm:min-w-[6.5rem]';
                        $filterChevron = 'pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-ui-faint';
                            ...
                            <div class="flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-3">
                                ...
                                <div class="{{ $filterBubble }} opacity-45" title="Coming soon">
                                    ...
                                    <div class="relative min-w-0 flex-1">
                                        ...
                                        <span class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-[10px] text-ui-faint" aria-hidden="true">▾</span>
                                    </div>
                                </div>
                                ...
                                @if (filled($filters['q']) || filled($filters['subject']) || filled($filters['year']) || ($filters['mode'] ?? 'books') !== 'books')
                                    <p class="w-full pb-0.5 sm:w-auto sm:self-center">
                                        <a href="{{ route('books.index') }}" class="text-[10px] font-semibold uppercase tracking-wider text-ui-faint underline decoration-parchment-4 dark:decoration-ink-4 underline-offset-4 hover:text-ui-primary hover:decoration-parchment-4 dark:decoration-ink-4">
                                            Clear filters
                                        </a>
                                    </p>
                                @endif
                        ...
                        @foreach ($books as $book)
                            <a
                                href="{{ route('books.show', $book) }}"
                                        class="group flex gap-4 rounded-[1.25rem] border border-ui bg-surface p-4 shadow-sm transition hover:border-border-strong"
                            >
                                <div class="shrink-0">
                                ...
                                <div class="min-w-0 flex-1 space-y-2">
                                            <h2 class="text-sm font-semibold leading-snug text-ui-primary group-hover:underline">
                                        {{ $book->title }}
                                    </h2>
